...
public class IncomeFragment extends Fragment {
    ...
    private void setupListeners() {
        fabAdd.setOnClickListener(v -> {
            IncomeUiState s = viewModel.getUiState().getValue();
    ...
    private void observeUiState() {
        viewModel.getUiState().observe(getViewLifecycleOwner(), state -> {
            ...
            tvMonthIncome.setText("Доходы за месяц: " + state.getAmountByMonth() + " ₽");
    ...
    private void showTransactionDialog(@Nullable Transaction transaction) {
        ...
        if (categoryNames.isEmpty()) {
            viewModel.loadData();
            viewModel.getUiState().observe(getViewLifecycleOwner(), new androidx.lifecycle.Observer<IncomeUiState>() {
                @Override
                public void onChanged(IncomeUiState newState) {
